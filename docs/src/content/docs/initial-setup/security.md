...
- The shipped local backend has no authentication or authorization layer. It now binds to `127.0.0.1` by default for that reason. If you override the bind host to expose it on your LAN, home server, or public interfaces, you are responsible for putting it behind your own access controls.
