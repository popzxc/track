...
const TASK_PARSER_SYSTEM_PROMPT: &str =
    "You convert short CLI issue notes into structured task data. Return only fields supported by the schema. Prefer concise, actionable descriptions.";

const TASK_PARSER_DEVELOPER_PROMPT: &str =
    "Choose priority from high, medium, or low. Default priority to medium when the text does not clearly set one. Choose project only from the provided project names and aliases. Return the chosen project name or alias exactly as selected. If the project is ambiguous or missing, output project as null and confidence as low. If you are uncertain about project selection, set confidence to low. Respond with strict JSON that matches the provided schema.";
